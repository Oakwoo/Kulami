...
from trainset_generator import TrainsetGenerator
...
def run_game():
    ...
    while iteration > 0:
        ...
        trainsetGenerator = TrainsetGenerator(gameStatus, board, Kulami_settings.trainset_address+"_size_"+str(board.width)+".txt", Kulami_settings.robot_turn)
        robot = Robot(gameStatus, board, screen, Kulami_settings.hole_size, "Minmax", Kulami_settings.robot_turn, Kulami_settings.robot_IQ)
        robot1 = Robot(gameStatus, board, screen, Kulami_settings.hole_size, "Minmax", 0, 3)
        while True:
            if gameStatus.turn == Kulami_settings.robot_turn:
                robot.play()
                trainsetGenerator.recordStatus()
                #continue
            else:
                robot1.play()
                #continue
            ...
            if gameStatus.end:
                ...
                if gameStatus.red_score > gameStatus.black_score:
                    ...
                    gameStatus.result = 2
                break
        if Kulami_settings.robot_turn == 0:
            trainsetGenerator.write(gameStatus.red_score - gameStatus.black_score)
        else:
            trainsetGenerator.write(gameStatus.black_score - gameStatus.red_score)
